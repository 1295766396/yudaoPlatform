<template>
  ...
  >
    <el-row class="mx-[-10px]">
      ...
      <el-col :span="24" class="px-10px">
        <el-form-item v-if="resetPasswordData.tenantEnable === 'true' || resetPasswordData.tenantEnable === true" prop="tenantName">
          <el-input
      ...
      <Verify
        ref="verify"
        v-if="resetPasswordData.captchaEnable === 'true' || resetPasswordData.captchaEnable === true"
...
const getCode = async () => {
  // 情况一，未开启：则直接发送验证码
  if (resetPasswordData.captchaEnable === 'false' || resetPasswordData.captchaEnable === false || !resetPasswordData.captchaEnable) {
    await getSmsCode({})
  } else {
    ...
    if (verify.value) {
      ...
      console.error('Verify component is not mounted')
      await getSmsCode({}) // 容错直接发送验证码
    }
  }
}

const getSmsCode = async (params) => {
  if (resetPasswordData.tenantEnable === 'true' || resetPasswordData.tenantEnable === true) {
    await getTenantId()
  }
