<template>
  ...
  >
    <el-row class="mx-[-10px]">
      ...
      <el-col :span="24" class="px-10px">
        <el-form-item v-if="registerData.tenantEnable === 'true' || registerData.tenantEnable === true" prop="tenantName">
          <el-input
      ...
      <Verify
        v-if="registerData.captchaEnable === 'true' || registerData.captchaEnable === true"
        ref="verify"
...
// 获取验证码
const getCode = async () => {
  const isValidate = await validForm()
  if (!isValidate) return
  // 情况一，未开启：则直接注册
  if (registerData.captchaEnable === 'false' || registerData.captchaEnable === false || !registerData.captchaEnable) {
...
const getTenantId = async () => {
  if (registerData.tenantEnable === 'true' || registerData.tenantEnable === true) {
